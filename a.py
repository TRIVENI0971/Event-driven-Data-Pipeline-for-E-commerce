...
def main():
    ...
    if "view_option" in st.session_state and st.session_state.view_option == "Performance Overview":
        ...
        if "page_orders" not in st.session_state:
            st.session_state.page_orders = 0  # Current page for Orders Table

        if "page_summary" not in st.session_state:
            st.session_state.page_summary = 0  # Current page for Order Summary

        
        LIMIT = 50  # Number of rows per page
        # BQ_ORDERS_TABLE = "your_project_id.your_dataset.orders"  # Replace with your actual BigQuery table name
        ...
        def pagination_controls(page_key, query_key, total_rows):
            col1, col2, col3 = st.columns([1, 1, 2])  # Adjust column widths

            unique_key = f"{query_key}_{page_key}"  # Ensure key uniqueness

            with col1:
                if st.button("Previous", key=f"prev_{unique_key}", disabled=st.session_state[page_key] == 0):
                    st.session_state[page_key] -= 1
                    st.rerun()  # Reload the fragment
            
            with col2:
                if st.button("Next", key=f"next_{unique_key}", disabled=st.session_state[page_key] >= (total_rows // LIMIT)):
                    st.session_state[page_key] += 1
                    st.rerun()  # Reload the fragment
            
            with col3:
                # Display the current row range
        ...
        #     with col1:
        #         if st.button("Previous", key=f"prev_{query_key}", disabled=st.session_state[page_key] == 0):
        #         # if st.button("Previous", key=f"prev_{query_key}_{page_key}", disabled=st.session_state[page_key] == 0):

        #             st.session_state[page_key] -= 1
        #             st.rerun()  # Reload the fragment
            
        #     with col2:
        #         if st.button("Next", key=f"next_{query_key}", disabled=st.session_state[page_key] >= (total_rows // LIMIT)):
        #             st.session_state[page_key] += 1
        #             st.rerun()  # Reload the fragment
    ...
    # Fetch total rows for Orders Table
        ...
        # Render pagination controls for Orders Table
        ...
        # Fetch total rows for Order Summary (same table for simplicity)
        ...
        # Render pagination controls for Order Summary
